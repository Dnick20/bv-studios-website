...
export class BotManager {
  ...
  /**
   * Convenience: register default wedding data bot if provided
   */
  registerWeddingDataBot(botClass, config = {}) {
    return this.registerBot('wedding-data', botClass, config)
  }
  ...
  async executeBot(botType, input = {}) {
    ...
    if (this.isCircuitOpen(circuitBreaker)) {
      const timeRemaining = Math.ceil(
        (circuitBreaker.timeout - (Date.now() - circuitBreaker.lastFailure)) /
          1000
      )
      throw new Error(
        `Bot ${botType} is temporarily disabled due to repeated failures. Retry in ${timeRemaining} seconds.`
      )
    }
  ...
  getBotInstance(botType) {
    ...
    if (!botInfo.instance) {
      try {
        ...
        throw new Error(
          `Failed to instantiate bot ${botType}: ${error.message}`
        )
      }
    }
  ...
  recordSuccess(botType) {
    ...
    if (circuitBreaker.state === 'half-open') {
      ...
      console.log(
        `Circuit breaker for ${botType} closed after successful execution`
      )
    }
  ...
  recordFailure(botType) {
    ...
    if (
      ...
      console.error(
        `Circuit breaker opened for ${botType} after ${circuitBreaker.failures} failures`
      )
    }
  }
  ...
  async getSystemHealth() {
    ...
    if (health.summary.unhealthy > 0 || health.summary.disabled > 0) {
      health.overall =
        health.summary.disabled === health.summary.total
          ? 'critical'
          : 'degraded'
    }
